...
class PetToysSpider(scrapy.Spider):
    ...
    # get slider images
    async def get_slider_images(self, page):
        # Slider container selector (replace if needed)
        slider_selector = '.img-box.swiper-container'
        await page.wait_for_selector(slider_selector)

        # Get all visible image elements within the slider initially
        # Target visible images
        visible_images = await page.query_selector_all(f"{slider_selector} div#banner-slider .img-thumb-inner > img")
        image_urls = []
        for image in visible_images:
            # Access 'src' attribute using evaluate
            # image_url = await image.evaluate("attr => attr.src")
            image_url = await image.evaluate('''el => el.dataset.original || el.src''')
    ...
    async def parse(self, response):
        ...
        for product in response.css('.products-item'):
            item = {
                ...
                'images': [],  # Add a list for image URLs
            }
            ...
            try:
                ...
                slider_container = await page.query_selector('.img-box.swiper-container')
                if slider_container:
                    # Extract image URLs from slider
                    imageurls = await self.get_slider_images(page)
                    item['images'] = imageurls
            except Exception as e:
                print(f"Error extracting slider images: {e}")
